...
class HumanoidEnvWrapper(gym.Wrapper):
...
if __name__ == "__main__":
    ...
    custom_env = False # Set to True if using a custom environment
    if custom_env:
        envs = gym.vector.AsyncVectorEnv(
            [lambda: HumanoidEnvWrapper(make_env(args.env, reward_scaling=args.reward_scale)) for _ in range(args.n_envs)])
    else:
        envs = gym.vector.AsyncVectorEnv(
